...
void main(){
  runApp(JobPaymentDetailTutor());
}

class JobPaymentDetailTutor extends StatefulWidget {
  final job;
  JobPaymentDetailTutor({@required job}) : this.job = job;

  @override
  _JobPaymentDetailTutorState createState() => _JobPaymentDetailTutorState();
}

class _JobPaymentDetailTutorState extends State<JobPaymentDetailTutor> {
  ...
  Future<void> openProposal(i)async{
    if(job['awarded'] == 'yes' && double.parse(job['amountChared']) > 0){
      return;
    }
    proposals[i]['amountCharged'] = double.parse(job['amountChared']);
    proposals[i]['direct_hiring'] = job['direct_hiring'];
  ...
  Future<void> requestSalary()async{
    try{
        ...
        Fluttertoast.showToast(
            msg: "Salary request sent to employer",
            toastLength: Toast.LENGTH_LONG,
            gravity: ToastGravity.BOTTOM,
            timeInSecForIosWeb: 1,
      ...
      if(_formKey.currentState.validate()){
        if(rating > 0){
          ...
          var data = {
            ...
            "to_user_id": job['posted_by'].toString(),
  ...
  Widget JobDetailsBody(){
    return ListView(
      children: [
        ...
        Padding(
          padding: EdgeInsets.all(15),
          child: Text(job['job_desc'], style: TextStyle(fontSize: 16,fontFamily: "Proxima"),),
        ),
        Padding(
        ...
        Container(
          ...
          padding: EdgeInsets.all(15),
          child: Text(job['direct_hiring'] == 'yes' ? "Proposal from client" : "Your proposal",style: TextStyle(fontFamily: "Proxima",fontSize: 16),),
        ),
        Container(
          child: Proposals(),
        ),
        ...
        Container(
          ...
          child: FlatButton(
            ...
            child: Text("Request Salary for ${job['payment_date']}" ,style: TextStyle(color: Colors.white,fontWeight: FontWeight.bold,fontFamily: "Proxima"),),
          ),
        )
            : Container(),
      ...
      return Column(
        ...
        children: [
          Container(
            ...
            padding: EdgeInsets.all(15),
            child: Text("Review from Client",style: TextStyle(fontFamily: "Proxima",fontSize: 16),),
          ),
          Container(
            child: InkWell(
              onTap: () {
//                openProposal(i);
  ...
  Widget Proposals(){
    ...
    for(var i=0; i<proposals.length; i++){
      if(job['direct_hiring'] == 'no' && userId != proposals[i]['user_id']){
        continue;
      }else if(amountCharged == null){
        setState(() {
          amountCharged = proposals[i]['original_charged'];
        });
      }
      list.add(
          Container(
                      ...
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: <Widget>[
                            ...
                            double.parse(job['amountChared']) > 0 ? Container() : Text("See more",style: TextStyle(color:appConfiguration.appColor,fontSize: 13,fontFamily: "Proxma")),
                            Padding(
